...
class AsyncTask:
    def __init__(self, args):
        ...
        self.metadata_scheme = MetadataScheme(
            args.pop()) if not args_manager.args.disable_metadata else MetadataScheme.LOCAL_IMAGE_GENERATOR
...
def worker():
    ...
    from modules.sdxl_styles import apply_style, get_random_style, local_image_generator_expansion, apply_arrays, random_style_name
    ...
    def progressbar(async_task, number, text):
        print(f'[Local Image Generator] {text}')
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras, persist_image=True) -> list:
        ...
        for index, x in enumerate(imgs):
            ...
            d = [('Prompt', 'prompt', task['log_positive_prompt']),
                 ...
                 ('Local Image Generator V2 Expansion', 'prompt_expansion', task['expansion']),
                 ('Styles', 'styles',
                  str(task['styles'] if not use_expansion else [local_image_generator_expansion] + task['styles'])),
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        if use_expansion:
            ...
            for i, t in enumerate(tasks):
                ...
                progressbar(async_task, current_progress, f'Preparing Local Image Generator text #{i + 1} ...')
    ...
    def handler(async_task: AsyncTask):
        ...
        if local_image_generator_expansion in async_task.style_selections:
            use_expansion = True
            async_task.style_selections.remove(local_image_generator_expansion)
        else:
            use_expansion = False

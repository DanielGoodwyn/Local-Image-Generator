# Local Image Generator on Docker
...
- A computer with specs good enough to run Local Image Generator, and proprietary Nvidia drivers
...
```sh
docker run -p 7865:7865 -v local_image_generator-data:/content/data -it \
--gpus all \
-e CMDARGS=--listen \
...
-e path_local_image_generator_expansion=/content/data/models/prompt_expansion/local_image_generator_expansion/ \
-e path_outputs=/content/app/outputs/ \
ghcr.io/lllyasviel/local_image_generator
```
### Running with Podman

```sh
podman run -p 7865:7865 -v local_image_generator-data:/content/data -it \
...
Your models and outputs are stored in the `local_image_generator-data` volume, which, depending on OS, is stored in `/var/lib/docker/volumes/` (or `~/.local/share/containers/storage/volumes/` when using `podman`).
...
Build with `docker`:
```sh
docker build . -t local_image_generator
```

Build with `podman`:
```sh
podman build . -t local_image_generator
```
...
When you are using `docker compose up` continuously, the container is not updated to the latest version of Local Image Generator automatically.
Run `git pull` before executing `docker compose build --no-cache` to build an image with the latest Local Image Generator version.
